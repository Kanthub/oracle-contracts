...
contract BLSApkRegistryTest is Test {
    ...
    function setUp() public {
        ...
        bytes memory data = abi.encodeWithSelector(BLSApkRegistry.initialize.selector, owner, whiteListManager, oracleManager);
    ...
    function testWhitelistAndPubkeyRegistration() public {
        // 构造一组bls公钥和签名
        BN254.G1Point memory msgHash = BN254.G1Point({ X: 21865530482018474585755963423619168960499467353431037262233960899024234433967, Y: 3539728706297954110075110532628922806564389507325174160259578565351636992365 });

        BN254.G1Point memory signature = BN254.G1Point({ X: 16098063868875928138477990443367166280697131948513748073012692220284750971347, Y: 15329843334933716897719702213638165599354107693369133305016255497531224599063 });

        BN254.G2Point memory pubKeyG2 = BN254.G2Point({ X: [17749938557935567636636221573629998351428902646807247102045274388891431404982, 4876593892177972657158095557941812406681431312843919154841886886892180250933], Y: [21414569513416923544024605285721660075805441825497220916794861514170708827362, 9375354739230872226547817213884458010371979876612785423004996479263944389946] });

        BN254.G1Point memory pubKeyG1 = BN254.G1Point({ X: 20443282814214573593856161370293586240075665004052083344248457337085485625356, Y: 13194418538414467055050033907265106142712607841859396823411150580302706299760 });
    ...
    function testAddAndRemoveBlsRegisterWhitelist() public {
        ...
        IBLSApkRegistry.PubkeyRegistrationParams memory params = IBLSApkRegistry.PubkeyRegistrationParams({ pubkeyG1: dummy, pubkeyG2: dummyG2, pubkeyRegistrationSignature: dummy });
    ...
    function testTrySignatureAndApkVerification() public view {
        ...
        // 4. 调用验证函数
        (bool pairingSuccessful, bool sigValid) = registry.trySignatureAndApkVerification(msgHash, pubKeyG1, pubKeyG2, signature);
