...
contract deployOraclePodScript is Script {
    ...
    function run() public {
        ...
        oraclePodAdmin.upgradeAndCall(ITransparentUpgradeableProxy(address(oraclePod)), address(oraclePodImplementation), abi.encodeWithSelector(OraclePod.initialize.selector, deployerAddress, oracleManagerAddr));
        ...
        string memory data = string(abi.encodePacked('{"proxyOraclePod": "', vm.toString(address(proxyOraclePod)), '", ', '"oraclePodImplementation": "', vm.toString(address(oraclePodImplementation)), '"}'));
