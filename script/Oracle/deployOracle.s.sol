...
contract deployOracleScript is Script {
    ...
    function run() public {
        ...
        blsApkRegistryProxyAdmin.upgradeAndCall(ITransparentUpgradeableProxy(address(blsApkRegistry)), address(blsApkRegistryImplementation), abi.encodeWithSelector(BLSApkRegistry.initialize.selector, deployerAddress, relayerManagerAddr, address(proxyOracleManager)));

        oracleManagerAdmin.upgradeAndCall(ITransparentUpgradeableProxy(address(oracleManager)), address(oracleManagerImplementation), abi.encodeWithSelector(OracleManager.initialize.selector, deployerAddress, proxyBlsApkRegistry, deployerAddress));
        ...
        string memory data = string(abi.encodePacked('{"proxyBlsApkRegistry": "', vm.toString(address(proxyBlsApkRegistry)), '", ', '"proxyOracleManager": "', vm.toString(address(proxyOracleManager)), '"}'));
