...
contract deployEventScript is Script {
    ...
    function run() public {
        ...
        TransparentUpgradeableProxy proxyEventManager = new TransparentUpgradeableProxy(address(emptyContract), deployerAddress, "");
        ...
        blsApkRegistryProxyAdmin.upgradeAndCall(ITransparentUpgradeableProxy(address(blsApkRegistry)), address(blsApkRegistryImplementation), abi.encodeWithSelector(BLSApkRegistry.initialize.selector, deployerAddress, relayerManagerAddr, address(proxyEventManager)));

        eventManagerAdmin.upgradeAndCall(ITransparentUpgradeableProxy(address(eventManager)), address(eventManagerImplementation), abi.encodeWithSelector(EventManager.initialize.selector, deployerAddress, proxyBlsApkRegistry, deployerAddress));
        ...
        string memory data = string(abi.encodePacked('{"proxyBlsApkRegistry": "', vm.toString(address(proxyBlsApkRegistry)), '", ', '"proxyEventManager": "', vm.toString(address(proxyEventManager)), '"}'));
